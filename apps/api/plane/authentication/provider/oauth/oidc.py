...
"""Generic OIDC OAuth2 provider (Authorization Code flow).

Endpoints are discovered from the issuer's /.well-known/openid-configuration, so
any standards-compliant IdP (Keycloak, Authentik, ...) works from just the issuer
URL plus a client id/secret.

The login nonce is persisted in the Redis cache keyed by the OAuth `state`, with a
session-cookie fallback: browsers commonly drop the session cookie on the
cross-site redirect back from the IdP, so a cookie-only nonce makes the callback
fail intermittently (the redirect-loop class of bug). Caching by `state` is the
reliable path; the session copy covers same-site flows.

Known limitations:
- ID token signature verification assumes RS256 (Keycloak's default alg).
- No PKCE yet; security rests on a confidential client (client_secret) + state + nonce.
"""
...
class OidcProvider(OauthAdapter):
    ...
    def __init__(self, request, code=None, state=None, callback=None):
        ...
        # Retrieve the nonce: cache (keyed by state) first, session as fallback.
        self.oidc_nonce = cache.get(f"oidc_nonce_{state}") if state else None
